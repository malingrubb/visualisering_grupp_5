...
const bredd = 700;
const höjd = 400;
const marginal = {top: 50, höger: 30, botten: 30, vänster: 60};
...
const svg = d3.select(".visual_5").append("svg")
  ...
  .append("g")
  .attr("transform", `translate(${marginal.vänster},${marginal.top + 30})`); // Adjust the value to move the graph down

// Title
svg.append("text")
  .attr("x", (bredd / 2))
  .attr("y", 0 - (marginal.top + 10)) // Adjust the value to position it above the legend
...
const legend = svg.append("g")
  .attr("transform", `translate(0, ${-marginal.top + 20})`); // Adjust the value to position it below the title
...
// Add the rest of your graph code here, such as axes, area chart, etc.
...
svg.selectAll(".layer")
  ...
  .style("fill-opacity", 0.2); // Add opacity to the fill color
